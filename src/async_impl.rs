...
impl YahooConnector {
    ...
    pub async fn get_summary<T: AsRef<str> + Display>(&self, ticker: &[T]) -> Result<YQuoteResponse, YahooError> {
        let url = format!(YQUOTE_QUERY!(), url = self.quote_url, symbols=ticker_as_list(ticker));
        YQuoteResponse::from_json(self.send_request(&url).await?)
    }
...
mod tests {
    ...
    fn test_quote_summary() {
        let provider = YahooConnector::new();
        let response =
            tokio_test::block_on(provider.get_summary(&["AAPL", "OKE"])).unwrap();
