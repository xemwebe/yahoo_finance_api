...
impl YahooConnector {
    ...
    pub async fn get_quote_range(
        ...
        YResponse::from_json(self.send_request_retry(&url).await?)?.map_error_msg()
    }
    ...
    async fn get_cookie(&mut self) -> Result<String, YahooError> {
        #[cfg(feature = "governor")]
        self.wait_for_rate_limit().await;
        let response = self.client.get(&self.cookie_url).send().await?;
        let status = response.status();
    ...
    async fn send_request_retry(&self, url: &str) -> Result<serde_json::Value, YahooError> {
        const MAX_RETRIES: usize = 1;
        let mut last_error = YahooError::NoResponse;
        for _attempt in 0..=MAX_RETRIES {
...
mod tests {
    ...
    /// Smoke test for the search endpoint across a basket of query strings.
    /// Prints per-query results instead of asserting, so a single
    /// rate-limited query (IP-level 429) does not fail the whole run.
    /// Run manually (not in CI):
    /// `cargo test --lib test_search_ticker_live -- --ignored --nocapture`
    #[ignore]
    #[test]
    fn test_search_ticker_live() {
        let queries = ["Apple", "Microsoft", "Tesla", "BTC-USD", "Vanguard"];
        for query in queries {
            let provider = YahooConnector::new().unwrap();
            let result = tokio_test::block_on(provider.search_ticker_opt(query));
            match result {
                Ok(res) => {
                    println!(
                        "{:>12}: count={}, quotes={}, news={}",
                        query,
                        res.count,
                        res.quotes.len(),
                        res.news.len()
                    );
                    for q in res.quotes.iter().take(3) {
                        println!("      {} ({}, {})", q.symbol, q.quote_type, q.exchange);
                    }
                }
                Err(e) => {
                    println!("{:>12}: FAILED {:?}", query, e);
                }
            }
        }
    }
    ...
    #[tokio::test]
    async fn test_retry_on_401_refreshes_crumb() {
        ...
        let result = conn.get_ticker_info("AAPL").await;
    ...
    #[tokio::test]
    async fn test_retry_on_403_refreshes_crumb() {
    ...
    #[tokio::test]
    async fn test_retry_on_html_response() {
        let mock = crate::mock_server::MockServer::start();
        queue_cookie(&mock);
        queue_crumb(&mock, "abc");
    ...
    #[tokio::test]
    async fn test_retry_on_api_error_invalid_crumb() {
        let mock = crate::mock_server::MockServer::start();
        queue_cookie(&mock);
        queue_crumb(&mock, "abc");
        mock.enqueue_plain(
    ...
    #[tokio::test]
    async fn test_get_crumb_refreshes_cookie_on_401() {
        ...
        let crumb = conn.get_crumb().await.unwrap();
        assert_eq!(crumb, "abc");
        let lines = mock.request_lines();
    ...
    #[tokio::test]
    async fn test_unauthorized_after_exhausted_retries() {
        let mock = crate::mock_server::MockServer::start();
        queue_cookie(&mock);
        queue_crumb(&mock, "abc");
        mock.enqueue(
        ...
        let lines = mock.request_lines();
        assert_eq!(lines.len(), 6);
        assert_eq!(lines[5].split_whitespace().next(), Some("GET"));
    }

    #[tokio::test]
    async fn test_bad_cookie_and_bad_crumb_recovers() {
    ...
    #[tokio::test]
    async fn test_get_crumb_refreshes_cookie_on_invalid_cookie_body() {
    ...
    #[tokio::test]
    async fn test_get_cookie_rate_limited() {
    ...
    #[tokio::test]
    async fn test_get_cookie_joins_multiple_set_cookie() {
    ...
    #[tokio::test]
    async fn test_cookie_header_value_keeps_all_cookies() {
    ...
    #[tokio::test]
    async fn test_get_cookie_429_wins_over_set_cookie() {
    ...
    #[tokio::test]
    async fn test_get_crumb_empty_body_exhausts_retries() {
        ...
        let result = conn.get_crumb().await;
    ...
    #[tokio::test]
    async fn test_get_crumb_too_many_requests_body_not_retried() {
    ...
    #[tokio::test]
    async fn test_get_crumb_html_body_refreshes_cookie() {
        // A maintenance HTML page served with a 200 status is not a valid
    ...
    #[tokio::test]
    async fn test_get_crumb_unauthorized_on_exhausted_401() {
    ...
    #[tokio::test]
    async fn test_get_crumb_401_no_extra_cookie_on_last_attempt() {
    ...
    #[tokio::test]
    async fn test_empty_quote_summary_returns_noresult() {
        let mock = crate::mock_server::MockServer::start();
        queue_cookie(&mock);
        queue_crumb(&mock, "abc");
        mock.enqueue_plain(200, "{}");
    ...
    #[tokio::test]
    async fn test_empty_symbol_rejected_without_requests() {
        let mock = crate::mock_server::MockServer::start();
        let mut conn = mock_connector(&mock);
        let result = conn.get_ticker_info("").await;
    ...
    #[tokio::test]
    async fn test_invalid_crumb_refreshes_and_succeeds() {
        let mut provider = YahooConnector::new().unwrap();
        ...
        let result = provider.get_ticker_info("AAPL").await;
        eprintln!("DIAG invalid_crumb result: {:?}", result);
        let result = result.unwrap();
        assert!(result.has_result());
    }

    #[tokio::test]
    async fn test_invalid_cookie_refreshes_and_succeeds() {
        let mut provider = YahooConnector::new().unwrap();
    ...
    #[tokio::test]
    async fn test_unknown_symbol_error_shape() {
        let mut provider = YahooConnector::new().unwrap();
        let result = provider.get_ticker_info("ZZZZX").await;
    ...
    #[tokio::test]
    async fn test_option_symbol_parses() {
        let mut provider = YahooConnector::new().unwrap();
        ...
        let mut provider = YahooConnector::new().unwrap();
        let result = provider.get_ticker_info(contract).await.unwrap();
    ...
    #[tokio::test]
    async fn test_crypto_null_trading_periods() {
        let mut provider = YahooConnector::new().unwrap();
        ...
        let result = provider.get_ticker_info("BTC-USD").await.unwrap();
        assert!(result.has_result());
    }
    ...
    #[tokio::test]
    async fn test_financial_events_success_mock() {
        let mock = crate::mock_server::MockServer::start();
        queue_cookie(&mock);
        queue_crumb(&mock, "abc");
        ...
        let mut conn = mock_connector(&mock);
        let events = conn.get_financial_events("AAPL", 25).await.unwrap();
    ...
    #[tokio::test]
    async fn test_financial_events_string_numeric_cells() {
    ...
    #[tokio::test]
    async fn test_financial_events_multiple_documents_aggregated() {
    ...
    #[tokio::test]
    async fn test_financial_events_401_retries() {
    ...
    #[tokio::test]
    async fn test_financial_events_403_refreshes_crumb_and_cookie() {
    ...
    #[tokio::test]
    async fn test_financial_events_500_retries_once() {
    ...
    #[tokio::test]
    async fn test_financial_events_429_not_retried() {
        ...
        let mut conn = mock_connector(&mock);
        let result = conn.get_financial_events("AAPL", 25).await;
        assert!(matches!(result, Err(YahooError::TooManyRequests(_))));
        assert_eq!(mock.request_lines().len(), 3);
    }

    #[tokio::test]
    async fn test_financial_events_api_error_invalid_crumb_exhausts() {
    ...
    #[tokio::test]
    async fn test_financial_events_api_error_reported() {
        ...
        let mut conn = mock_connector(&mock);
        let result = conn.get_financial_events("NOPE", 25).await;
        assert!(matches!(result, Err(YahooError::ApiError(_))));
        assert_eq!(mock.request_lines().len(), 3);
    }

    #[tokio::test]
    async fn test_financial_events_parse_error_refreshes_crumb_and_cookie() {
    ...
    #[tokio::test]
    async fn test_json_error_refreshes_cookie_and_crumb() {
    ...
    #[tokio::test]
    async fn test_get_crumb_404_returns_fetch_failed() {
    ...
    #[tokio::test]
    async fn test_get_crumb_500_500_returns_server_error() {
    ...
    #[tokio::test]
    async fn test_json_error_code_only_invalid_crumb_retries() {
    ...
    #[tokio::test]
    async fn test_get_crumb_5xx_retries_once() {
    ...
    #[tokio::test]
    async fn test_financial_events_null_result_with_error_retries() {
    ...
    #[tokio::test]
    async fn test_chart_retry_on_html_response() {
        ...
        let result = conn.get_quote_range("TEST", "1d", "5d").await;
        assert!(result.is_ok(), "expected Ok, got {:?}", result.err());
        assert_eq!(mock.request_lines().len(), 2);
    }

    #[tokio::test]
    async fn test_chart_retry_on_empty_response() {
    ...
    #[tokio::test]
    async fn test_chart_retry_on_server_error() {
    ...
    #[tokio::test]
    async fn test_chart_retry_exhausted() {
